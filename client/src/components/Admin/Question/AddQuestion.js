import { Box, Typography,TextField, Button, MenuItem, RadioGroup, Radio, FormControlLabel} from "@mui/material";
...
function Content() {
  ...
  return (
    ...
    >
      ...
      <Box
        ...
        sx={{
          ...
          borderRadius: "30px",
          width: "60%",
          '& .MuiTextField-root': {ml: 9, width: '80%', fontFamily: 'Poppins'},
        }}
      >
        <br></br>
    ...
    <FormControlLabel  sx={{color: "#000"}} onClick={() => setquesType(true)} value="MCQ" control={<Radio sx={{'&.Mui-checked': {color: "#edea0f" }}} />} label="Multiple Choice Question (MCQ)" />
    <FormControlLabel sx={{color: "#000"}} onClick={() => setquesType(false)} value="fill" control={<Radio sx={{'&.Mui-checked': {color: "#edea0f" }}} />} label="Fill in the Blanks" />
  </RadioGroup>
      <br></br>
      { quesType ? 
      <Box sx={{display: "grid", paddingLeft: '7%', gridTemplateColumns: "auto auto auto auto", '& .MuiTextField-root': {width: '70%', margin: "1%", fontFamily: 'Poppins'},}}>
      <FTypography > Option 1</FTypography>
        ...
        <Button sx={{width: "100%"}} onClick={() => setQues("Question Added Successfully!")}>
          <Badge
            content="Create"
            logout
            style={{
